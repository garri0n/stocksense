...
if (!isProduction) {
  ...
  // IMPORTANT: Remove any quotes - the test shows it works WITHOUT quotes!
  const cleanUsername = rawUsername.replace(/['`"]/g, '');
  
  // NO QUOTES - just the clean username
  const formattedUser = cleanUsername;
  
  console.log('Username format (no quotes):', formattedUser);

  pool = mysql.createPool({
    host: process.env.TIDB_HOST,
    user: formattedUser,  // Just the username, no quotes!
